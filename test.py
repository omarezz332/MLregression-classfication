...
combined_data = pd.read_csv('test.csv')

combined_data=combined_data.drop(['homepage','id','original_language','original_title','status','title','movie_id'],axis=1)

combined_data.corr()['vote_average']

def remove_html(text):
    soup = BeautifulSoup(text,'xlml')
...
combined_data['overview'] = combined_data['overview'].apply(lambda x: remove_punctuation(combined_data['overview']))

combined_data['tagline'] = combined_data['tagline'].apply(lambda x: remove_punctuation(combined_data['tagline']))
